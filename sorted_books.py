...
class Room:
    ...
    def ensure_category_assignment(self, category: str) -> Shelf:
        ...
        # Не знайдено — вибираємо полицю з найменшою кількістю категорій (детерміновано).
        if not self.shelves:
...
# ---- Step 1: Організація книжок за категоріями ----
...
#сортування ще не робимо
if __name__ == "__main__":
    room = Room(name="Bob's Room", shelves=[Shelf("A"), Shelf("B")])

    pile = {
        Book("Dune", "Frank Herbert", "Sci-Fi"),
        Book("Neuromancer", "William Gibson", "Sci-Fi"),
        Book("Sapiens", "Yuval Noah Harari", "Non-Fiction"),
        Book("1984", "George Orwell", "Dystopia"),
        Book("Animal Farm", "George Orwell", "Dystopia"),
    }

    mapping = organize_by_category(pile, room)

    print("Category → Shelf:")
    for cat, sh in sorted(mapping.items()):
        print(f"  {cat:12} → {sh.name}")
